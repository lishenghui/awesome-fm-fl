...
import types
import re
from entry_table import PaperInfo, PaperTable, ResourcePaperTable, convert_entry
        ...
        for k, v in config.items():
            ...
            if paper_item:
                ...
                def custom_title(text):
                    words = text.split()
                    title_cased_words = [word.title() if not word.isupper() else word
                                         for word in words]
...
input_file = '../bibs/references.bib'
output_file = '../README.rst'
config_file = 'contents.yaml'
convert_bibtex(input_file, output_file, config_file)
